      ...
      <div className="max-w-7xl mx-auto py-20 px-4 md:px-8 lg:px-10">
        <div className="flex gap-2 items-center mb-4 w-full justify-between">
          ...
          <button
            onClick={() => {
              try {
                downloadPDF("Resume.pdf", "Resume.pdf");
                toast.success("Resume Downloaded!");
              } catch (e) {
                toast.error("Something went wrong!");
              }
            }}
            className="bg-red-600 flex px-3 py-2 justify-center items-center rounded-md"
          >
            Resume.pdf
            <BiDownload className="ml-2 shrink-0 size-6" />
          </button>
        </div>

        <p className="text-neutral-300 text-sm md:text-base max-w-sm">
          Experienced in building scalable solutions, optimizing architectures,
